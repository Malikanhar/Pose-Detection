...
class Human:
    ...
    def get_coor(self):
        coor = np.zeros((2, 18))
        for i, x in self.body_parts.items():
            coor[0, i] = x.x
            coor[1, i] = x.y
        return coor
...
class BodyPart:
    ...
    def __str__(self):
        return 'BodyPart:%d-(%.2f, %.2f) score=%.2f' % (self.part_idx, self.x * 53, self.y * 45, self.score)
...
class TfPoseEstimator:
    ...
    @staticmethod
    def draw_humans(npimg, humans, imgcopy=False, kelas=None):
        if imgcopy:
            npimg = np.copy(npimg)
        ...
        for human in humans:
            ...
            (tw, th) = cv2.getTextSize(text_kelas, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]
            box = ((center_mata[0], center_mata[1] + 5), (center_mata[0] + tw, center_mata[1] - th - 2))
            cv2.rectangle(npimg, box[0], box[1], (255,255,255), cv2.FILLED)
            npimg = cv2.putText(npimg, text_kelas, tuple(center_mata), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0))
            h_id+=1
             # --------------------------------------#
        return npimg
